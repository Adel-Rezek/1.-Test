//Console.Write("Geben Sie bitte den erste Zahl ein: ");
//string ersteZahl = Console.ReadLine();
//Console.WriteLine();
//Console.Write("Geben Sie bitte den zweiter Zahl ein: ");
//string zweiteZahl = Console.ReadLine();
//Console.WriteLine();

//int ersterZahlAlsInt = int.Parse(ersteZahl);
//int zweiterzahlAlsInt = int.Parse(zweiteZahl);

//int ergibnis1 = ersterZahlAlsInt + zweiterzahlAlsInt;
//Console.WriteLine($"1. addieren -> Ergibnis ist: {ersterZahlAlsInt} + {zweiterzahlAlsInt} = {ergibnis1}");
//Console.WriteLine();


//int ergibnis2 = ersterZahlAlsInt - zweiterzahlAlsInt;
//Console.WriteLine($"2. substrahieren -> Ergibnis ist: {ersterZahlAlsInt} - {zweiterzahlAlsInt} = {ergibnis2}");
//Console.WriteLine();


//int ergibnis3 = ersterZahlAlsInt * zweiterzahlAlsInt;
//Console.WriteLine($"3. multiplizieren -> Ergibnis ist: {ersterZahlAlsInt} * {zweiterzahlAlsInt} = {ergibnis3}");
//Console.WriteLine();


////int ergibnis4 = ersterZahlAlsInt  / zweiterzahlAlsInt;
////Console.WriteLine($"4. Ergibnis ist: {ersterZahlAlsInt} / {zweiterzahlAlsInt} = {ergibnis4}");
//float floatErgibnis4 = (float)ersterZahlAlsInt / (float)zweiterzahlAlsInt;
//Console.WriteLine($"4. division -> Ergibnis ist: {(float)ersterZahlAlsInt} / {(float)zweiterzahlAlsInt} = {floatErgibnis4}");


Console.Write("Geben Sie bitte den erste Zahl ein: ");
string ersteZahl = Console.ReadLine();
Console.WriteLine();
Console.Write("Geben Sie bitte den zweiter Zahl ein: ");
string zweiteZahl = Console.ReadLine();
Console.WriteLine();

double ersterZahlAlsInt = double.Parse(ersteZahl);
double zweiterzahlAlsInt = double.Parse(zweiteZahl);

double ergibnis1 = ersterZahlAlsInt + zweiterzahlAlsInt;
Console.WriteLine($"1. addieren -> Ergibnis ist: {ersterZahlAlsInt} + {zweiterzahlAlsInt} = {ergibnis1}");
Console.WriteLine();


double ergibnis2 = ersterZahlAlsInt - zweiterzahlAlsInt;
Console.WriteLine($"2. substrahieren -> Ergibnis ist: {ersterZahlAlsInt} - {zweiterzahlAlsInt} = {ergibnis2}");
Console.WriteLine();


double ergibnis3 = ersterZahlAlsInt * zweiterzahlAlsInt;
Console.WriteLine($"3. multiplizieren -> Ergibnis ist: {ersterZahlAlsInt} * {zweiterzahlAlsInt} = {ergibnis3}");
Console.WriteLine();


double ergibnis4 = ersterZahlAlsInt / zweiterzahlAlsInt;
Console.WriteLine($"4. division -> Ergibnis ist: {ersterZahlAlsInt} / {zweiterzahlAlsInt} = {ergibnis4}");



